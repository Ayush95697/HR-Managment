...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      drawBlob(b1x, b1y, 800, isLight ? 'rgba(59, 130, 246, 0.08)' : 'rgba(37, 99, 235, 0.15)', 'transparent');

      // Blob 2 (Cyan)
      const b2x = width * 0.7 + Math.cos(time * 1.2) * 350 + px * 1.2;
      const b2y = height * 0.6 + Math.sin(time * 0.9) * 250 + py * 1.2;
      drawBlob(b2x, b2y, 700, isLight ? 'rgba(6, 182, 212, 0.08)' : 'rgba(6, 182, 212, 0.15)', 'transparent');

      // Central Spotlight
      const cx = width / 2;
      const cy = height / 2;
      drawBlob(cx, cy, 600 + Math.sin(time * 2) * 50, isLight ? 'rgba(59, 130, 246, 0.05)' : 'rgba(59, 130, 246, 0.08)', 'transparent');

      // Cursor Glow
      drawBlob(mouse.x, mouse.y, 400, isLight ? 'rgba(59, 130, 246, 0.05)' : 'rgba(59, 130, 246, 0.08)', 'transparent');
